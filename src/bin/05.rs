...
impl From<&str> for Input2 {
    fn from(value: &str) -> Self {
        ...
        let mut copy_value_lines = value.lines();
        while let Some(s) = copy_value_lines.next() {
        ...
        Self { ranges: valid, available: avail }
    }
}

fn try_join_ranges(r1: &Range<u64>, r2:& Range<u64>) -> Result<Range<u64>, (Range<u64>, Range<u64>)> {
    ...
    match (is_r1_start_in_r2, is_r1_end_in_r2, is_r2_start_in_r1, is_r2_end_in_r1) {
...
fn reduce_ranges(acc: Vec<Range<u64>>) -> Vec<Range<u64>> {
    let next: Vec<Range<u64>> = acc.chunks(2)
        .flat_map(|c| {
            if c.len() > 1 {
                let res: Result<Range<u64>, (Range<u64>, Range<u64>)> = try_join_ranges(&c[0], &c[1]);
                match res {
                    Ok(r) => vec![r],
                    Err((r1, r2)) => vec![r1, r2],
                }
            } else {
                let res: Vec<Range<u64>> = c.iter().cloned().collect();
                res
            }
        }).collect();
    let next2: Vec<Range<u64>> = next.rchunks(2)
        .flat_map(|c| {
            if c.len() > 1 {
                let res: Result<Range<u64>, (Range<u64>, Range<u64>)> = try_join_ranges(&c[0], &c[1]);
                match res {
...
fn fold_reduce_ranges(acc: Vec<Range<u64>>, curr: &Range<u64>) -> Vec<Range<u64>> {
    let mut next: Vec<Range<u64>> = Vec::new();
    let mut init = curr.start..curr.end;
    let start_acc_len = &acc.len();
    for r in acc {
        match classify_any(Some(r.start), Some(r.end), Some(init.start), Some(init.end), false) {
            ...
            RangeOverlap::None => { next.push(r); }
            RangeOverlap::AContainsB => { next.push(r); },
        };
    }
    next.push(init);
    if &next.len() == start_acc_len {
        ...
        fold_reduce_ranges(next, curr)
    }
}

fn get_overlaps(ranges: &HashMap<u64, u64>, start: u64, end: u64, existing: Vec<u64>) -> Vec<(Vec<u64>, (u64, u64))> {
